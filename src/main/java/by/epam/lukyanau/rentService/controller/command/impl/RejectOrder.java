...
public class RejectOrder implements Command {
    ...
    public Router execute(HttpServletRequest request) {
        ...
        try {
            ...
            if (foundOrder.isPresent() && foundOrder.get().getStatus() != Order.Status.REJECTED) {
                orderService.updateOrderStatus(idOrder, Order.Status.REJECTED.getNameStatus());
                request.setAttribute(MessageAttribute.REJECT_ORDER_ID, idOrder);
